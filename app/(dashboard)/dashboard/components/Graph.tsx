...
import { MetricType, StockMetrics, StockBottleType } from '../types';
        ...
        callbacks: {
          label: (item: TooltipItem<'bar'>) => {
            ...
            return config.isCurrency
              ? `${label ?? title}: Rs ${value.toLocaleString()}`
              : `${label ?? title}: ${value}${config.unit}`;
          },
        },
      },
    },
    ...
    >
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-bold text-lg text-slate-900">{label ?? title}</h3>
        </div>
